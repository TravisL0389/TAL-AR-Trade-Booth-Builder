import { V as SerializationHelper, Y as serialize, at as serializeAsVector3, b as TimingTools, et as serializeAsMatrix, lt as __esDecorate, ut as __runInitializers } from "./guid-B933-psj.js";
import { f as Observable, i as TmpVectors, o as Vector3, t as Matrix } from "./math.vector.pure-ykbdAlJz.js";
import { n as RegisterClass, t as GetClass } from "./typeStore-D4_ZTXSQ.js";
import { t as BaseTexture } from "./baseTexture.pure-PofqupeU.js";
import { t as Texture } from "./texture.pure-C6Ruu6lz.js";
import { t as RegisterAbstractEngineCubeTexture } from "./abstractEngine.cubeTexture.pure-9TMvIiCN.js";
//#region node_modules/@babylonjs/core/Materials/Textures/cubeTexture.pure.js
/** This file must only contain pure code and pure imports */
var DefaultLodScale = .8;
/**
	...
	return _a = class CubeTexture extends _classSuper {
		...
		*/
		updateURL(url, forcedExtension = null, onLoad = null, prefiltered = false, onError = null, extensions = null, delayLoad = false, files = null, buffer = null) {
			...
			this.url = url;
			if (forcedExtension) this._forcedExtension = forcedExtension;
			...
			if (files) this._files = files;
				...
				if (!isBasis && !isEnv && !isDDS && !extensions) extensions = [
					"_px.jpg",
					"_py.jpg",
					"_pz.jpg",
					"_nx.jpg",
				...
				if (extensions) {
					for (let index = 0; index < extensions.length; index++) this._files.push(url + extensions[index]);
			...
			} else this._loadTexture(onLoad, onError);
		}
		/**
		...
		*/
		setReflectionTextureMatrix(value) {
			if (value.updateFlag === this._textureMatrix.updateFlag) return;
			if (value.isIdentity() !== this._textureMatrix.isIdentity()) this.getScene()?.markAllMaterialsAsDirty(1, (mat) => mat.getActiveTextures().indexOf(this) !== -1);
			this._textureMatrix = value;
			if (!this.getScene()?.useRightHandedSystem) return;
		...
		*/
		...
		_loadTexture(onLoad = null, onError = null) {
			...
			if (!this._texture) {
				if (this._prefiltered) this._texture = this._getEngine().createPrefilteredCubeTexture(this.url, scene, this._lodScale, this._lodOffset, onLoad, errorHandler, this._format, this._forcedExtension, this._createPolynomials);
				else this._texture = this._getEngine().createCubeTexture(this.url, scene, this._files, this._noMipmap, onLoad, errorHandler, this._format, this._forcedExtension, false, this._lodScale, this._lodOffset, null, this._loaderOptions, !!this._useSRGBBuffer, this._buffer);
				this._texture?.onLoadedObservable.add(() => this.onLoadObservable.notifyObservers(this));
			} else if (this._texture.isReady) TimingTools.SetImmediate(() => onLoadProcessing());
			else this._texture.onLoadedObservable.add(() => onLoadProcessing());
		}
		/**
		...
		*/
		clone() {
			...
			if (savedIrradianceTexture && !this.irradianceTexture) this.irradianceTexture = savedIrradianceTexture;
		...
		__esDecorate(_a, null, _get_boundingBoxSize_decorators, {
			kind: "getter",
			name: "boundingBoxSize",
			static: false,
			private: false,
			access: {
				has: (obj) => "boundingBoxSize" in obj,
				get: (obj) => obj.boundingBoxSize
			},
			metadata: _metadata
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _set_rotationY_decorators, {
			kind: "setter",
			name: "rotationY",
			static: false,
			private: false,
			access: {
				has: (obj) => "rotationY" in obj,
				set: (obj, value) => {
					obj.rotationY = value;
				}
			},
		...
		__esDecorate(null, null, _boundingBoxPosition_decorators, {
			kind: "field",
			name: "boundingBoxPosition",
			static: false,
			private: false,
			access: {
				has: (obj) => "boundingBoxPosition" in obj,
				get: (obj) => obj.boundingBoxPosition,
				set: (obj, value) => {
					obj.boundingBoxPosition = value;
				}
			},
			metadata: _metadata
		}, _boundingBoxPosition_initializers, _boundingBoxPosition_extraInitializers);
		__esDecorate(null, null, __files_decorators, {
			kind: "field",
			name: "_files",
			static: false,
			private: false,
			access: {
				has: (obj) => "_files" in obj,
				get: (obj) => obj._files,
				set: (obj, value) => {
					obj._files = value;
				}
			...
			metadata: _metadata
		}, __files_initializers, __files_extraInitializers);
		__esDecorate(null, null, __forcedExtension_decorators, {
			kind: "field",
			name: "_forcedExtension",
			static: false,
			private: false,
			access: {
				has: (obj) => "_forcedExtension" in obj,
				get: (obj) => obj._forcedExtension,
				set: (obj, value) => {
					obj._forcedExtension = value;
				}
			},
			metadata: _metadata
		}, __forcedExtension_initializers, __forcedExtension_extraInitializers);
		__esDecorate(null, null, __extensions_decorators, {
			kind: "field",
			name: "_extensions",
			static: false,
			private: false,
			access: {
				has: (obj) => "_extensions" in obj,
				get: (obj) => obj._extensions,
				set: (obj, value) => {
					obj._extensions = value;
				}
			},
			metadata: _metadata
		}, __extensions_initializers, __extensions_extraInitializers);
		__esDecorate(null, null, __textureMatrix_decorators, {
			kind: "field",
			name: "_textureMatrix",
			static: false,
			private: false,
			access: {
				has: (obj) => "_textureMatrix" in obj,
				get: (obj) => obj._textureMatrix,
				set: (obj, value) => {
					obj._textureMatrix = value;
				}
			},
			metadata: _metadata
		}, __textureMatrix_initializers, __textureMatrix_extraInitializers);
		__esDecorate(null, null, __textureMatrixRefraction_decorators, {
			kind: "field",
			name: "_textureMatrixRefraction",
			static: false,
			private: false,
			access: {
				has: (obj) => "_textureMatrixRefraction" in obj,
				get: (obj) => obj._textureMatrixRefraction,
				set: (obj, value) => {
					obj._textureMatrixRefraction = value;
				}
			},
			metadata: _metadata
		}, __textureMatrixRefraction_initializers, __textureMatrixRefraction_extraInitializers);
...
function CubeTextureCreateFromImages(files, scene, noMipmap) {
	let rootUrlKey = "";
	for (const url of files) rootUrlKey += url;
...
function CubeTextureParse(parsedTexture, scene, rootUrl) {
	const texture = SerializationHelper.Parse(() => {
		...
		if (parsedTexture.prefiltered) prefiltered = parsedTexture.prefiltered;
	...
	if (parsedTexture.boundingBoxPosition) texture.boundingBoxPosition = Vector3.FromArray(parsedTexture.boundingBoxPosition);
	if (parsedTexture.boundingBoxSize) texture.boundingBoxSize = Vector3.FromArray(parsedTexture.boundingBoxSize);
	if (parsedTexture.animations) for (let animationIndex = 0; animationIndex < parsedTexture.animations.length; animationIndex++) {
...
RegisterCubeTexture();
//#endregion
export { CubeTexture, CubeTextureCreateFromImages, CubeTextureCreateFromPrefilteredData, CubeTextureParse, RegisterCubeTexture };
